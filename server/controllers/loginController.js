...
module.exports = {
    register: async (req, res) => {
        ...
        const registerUser = await db.find_user({ username })
        if (registerUser[0]) {
            return res.status(200).send({ message: 'Username is taken' })
        }
        const salt = bcrypt.genSaltSync(10);
        const hash = bcrypt.hashSync(password, salt);
        let newUser = await db.create_user({ username, email, hash })
        req.session.user = { id: newUser[0].id, username: newUser[0].username, email: newUser[0].email }
